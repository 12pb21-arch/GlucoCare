...
(function () {
  ...
  var STORAGE_KEY = 'glucocare_patients_v5';
  ...
  function openAddRecord(card) {
    ...
    var body =
      '<div class="modal-grid">' +
        ...
        '<label class="modal-label" style="grid-column:1/-1">❤️ Heart rate (BPM)<input class="modal-input" id="rHeartRate" type="number" min="30" max="220" placeholder="e.g. 72" /></label>' +
      '</div>' +
    ...
    createModal('📋 Add Health Record', body, function (overlay) {
      ...
      var heartRate = overlay.querySelector('#rHeartRate').value.trim();
      ...
      var store     = loadStore();
      var patient   = getPatient(store, patientId);
      patient.records.push({ date: dateVal, fasting: fasting, after: after, bpMorn: bpMorn, bpEve: bpEve, heartRate: heartRate });
      saveStore(store);
